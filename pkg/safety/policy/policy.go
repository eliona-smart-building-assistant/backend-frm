...
type BaseFailurePolicy[R any] struct {
	...
	onSuccess         func(safety.ExecutionEvent[R])
	onFailure         func(safety.ExecutionEvent[R])
}
...
func (p *BaseFailurePolicy[R]) OnSuccess(listener func(event safety.ExecutionEvent[R])) {
	p.onSuccess = listener
}

func (p *BaseFailurePolicy[R]) OnFailure(listener func(event safety.ExecutionEvent[R])) {
...
func (d *BaseDelayablePolicy[R]) WithDelayFunc(delayFunc safety.DelayFunc[R]) {
	d.DelayFunc = delayFunc
}

// ComputeDelay returns a computed delay else -1 if no delay could be computed.
func (d *BaseDelayablePolicy[R]) ComputeDelay(exec safety.ExecutionAttempt[R]) time.Duration {
