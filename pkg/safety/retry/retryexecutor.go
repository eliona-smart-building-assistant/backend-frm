...
import (
	...
	"github.com/eliona-smart-building-assistant/backend-frm/pkg/safety/util"
)
...
func (e *executor[R]) Apply(innerFn func(safety.Execution[R]) *common.PolicyResult[R]) func(safety.Execution[R]) *common.PolicyResult[R] {
	return func(exec safety.Execution[R]) *common.PolicyResult[R] {
		...
		for {
			...
			if e.onRetryScheduled != nil {
				e.onRetryScheduled(safety.ExecutionScheduledEvent[R]{
			...
			if e.budget != nil && !e.budget.TryAcquirePermit() {
				...
				return safety.FailureResult[R](budget.ErrExceeded)
			}

			if e.onRetry != nil {
				e.onRetry(safety.ExecutionEvent[R]{ExecutionAttempt: execInternal.CopyWithResult(result)})
			}
...
func (e *executor[R]) OnFailure(exec policy.ExecutionInternal[R], result *common.PolicyResult[R]) *common.PolicyResult[R] {
	...
	if isAbortable && e.onAbort != nil {
		e.onAbort(safety.ExecutionEvent[R]{ExecutionAttempt: exec.CopyWithResult(result)})
	}
	if e.retriesExceeded {
		if !isAbortable && e.onRetriesExceeded != nil {
			e.onRetriesExceeded(safety.ExecutionEvent[R]{ExecutionAttempt: exec.CopyWithResult(result)})
		}
		if !e.returnLastFailure {
			return safety.FailureResult[R](ExceededError{
...
func (e *executor[R]) getDelay(exec safety.ExecutionAttempt[R]) time.Duration {
...
func (e *executor[R]) getFixedOrRandomDelay(exec safety.ExecutionAttempt[R]) time.Duration {
	if e.Delay != 0 {
		// Adjust for backoffs
