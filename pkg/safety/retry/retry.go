...
import (
	...
	"github.com/eliona-smart-building-assistant/backend-frm/pkg/safety/budget"
	"github.com/eliona-smart-building-assistant/backend-frm/pkg/safety/internal"
	"github.com/eliona-smart-building-assistant/backend-frm/pkg/safety/policy"
)
...
type Builder[R any] interface {
	safety.FailurePolicyBuilder[Builder[R], R]
	safety.DelayablePolicyBuilder[Builder[R], R]
	...
	OnAbort(listener func(safety.ExecutionEvent[R])) Builder[R]
	...
	OnRetryScheduled(listener func(safety.ExecutionScheduledEvent[R])) Builder[R]

	// OnRetry registers the listener to be called when a retry is about to be attempted.
	OnRetry(listener func(safety.ExecutionEvent[R])) Builder[R]
	...
	OnRetriesExceeded(listener func(safety.ExecutionEvent[R])) Builder[R]
...
type config[R any] struct {
	...
	onAbort           func(safety.ExecutionEvent[R])
	onRetry           func(safety.ExecutionEvent[R])
	onRetryScheduled  func(safety.ExecutionScheduledEvent[R])
	onRetriesExceeded func(safety.ExecutionEvent[R])
}
...
func (c *config[R]) WithDelayFunc(delayFunc safety.DelayFunc[R]) Builder[R] {
...
func (c *config[R]) OnSuccess(listener func(event safety.ExecutionEvent[R])) Builder[R] {
	c.BaseFailurePolicy.OnSuccess(listener)
	return c
}

func (c *config[R]) OnFailure(listener func(event safety.ExecutionEvent[R])) Builder[R] {
	c.BaseFailurePolicy.OnFailure(listener)
	return c
}

func (c *config[R]) OnAbort(listener func(safety.ExecutionEvent[R])) Builder[R] {
	c.onAbort = listener
	return c
}

func (c *config[R]) OnRetry(listener func(safety.ExecutionEvent[R])) Builder[R] {
	c.onRetry = listener
	return c
}

func (c *config[R]) OnRetryScheduled(listener func(safety.ExecutionScheduledEvent[R])) Builder[R] {
	c.onRetryScheduled = listener
	return c
}

func (c *config[R]) OnRetriesExceeded(listener func(safety.ExecutionEvent[R])) Builder[R] {
